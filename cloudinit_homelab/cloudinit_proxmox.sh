...
download_os() {
    case $os_choice in
        "Debian")
            if [ ! -e "debian-12-generic-amd64.qcow2" ]; then
            ...
            ;;
        "Ubuntu")
            if [ ! -e "ubuntu-22.04-server-cloudimg-amd64.img" ]; then
...
qm disk resize "$vm_number" --scsi0 "$disk_size" || { echo "Error setting boot disk"; exit 1; }
